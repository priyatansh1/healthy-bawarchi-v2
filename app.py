...
# ─────────────────────────────────────────────
# DESIGN SYSTEM — cream + deep green + saffron
# ─────────────────────────────────────────────
st.markdown("""
...
.hb-season-pill {
    font-size: 11px;
    color: var(--leaf);
    background: rgba(244, 201, 123, 0.15);
    padding: 4px 10px;
    border-radius: 999px;
    letter-spacing: 0.5px;
    white-space: nowrap;
...
.hb-chip-peak {
    background: rgba(244, 201, 123, 0.20);
    color: var(--saffron-soft);
    padding: 6px 14px;
    border-radius: 999px;
    font-size: 13px;
...
.hb-season-tip {
    margin-top: 14px;
    padding-top: 12px;
    border-top: 1px solid rgba(244, 201, 123, 0.15);
    font-size: 12px;
    color: var(--leaf);
    font-style: italic;
    line-height: 1.5;
...
.stTextArea textarea:focus, .stTextInput input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(31, 77, 44, 0.10) !important;
}

/* RADIO (cuisine selector) */
...
@media (max-width: 600px) {
    .hb-hero { padding: 1.75rem 1.25rem; }
    .hb-card { padding: 1.25rem; }
    .hb-recipe-head { padding: 1.25rem; }
    .hb-recipe-title { font-size: 22px; }
    .hb-callout { margin-left: 1.25rem; margin-right: 1.25rem; }
}
...
<div class="hb-hero">
  <div class="hb-brand">
    <div class="hb-brand-mark">HB</div>
    <div>
      <div class="hb-brand-text-en">Healthy Bawarchi</div>
      <div class="hb-brand-text-ur">{_hero_eyebrow_ur}</div>
    </div>
  </div>
  {_hero_title_html}
  <div class="hb-hero-sub">{_hero_sub}</div>
  <div class="hb-stats">
    <div class="hb-stat">
      <div class="hb-stat-number">{_visits}</div>
      <div class="hb-stat-label">{_stat_visits}</div>
    </div>
    <div class="hb-stat">
      <div class="hb-stat-number accent">{_recipes}</div>
      <div class="hb-stat-label">{_stat_recipes}</div>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
st.markdown(f"""
<div class="hb-season">
  <div class="hb-season-head">
    <div>
      <div class="hb-season-title">{_ss_title}</div>
      <div class="hb-season-sub">{_ss_sub}</div>
    </div>
    <div class="hb-season-pill">{_ss_pill}</div>
  </div>
  <div class="hb-season-chips">{''.join(_chips)}</div>
  <div class="hb-season-tip">{_ss_tip}</div>
</div>
""", unsafe_allow_html=True)
...
if recipe:
    ...
    st.markdown(f"""
<div class="hb-recipe">
  <div class="hb-recipe-head">
    <div class="hb-recipe-eyebrow">{_eyebrow}</div>
    <div class="hb-recipe-title">{name}</div>
    <div class="hb-recipe-desc">{desc}</div>
    <div class="hb-recipe-meta">
      <span class="hb-recipe-meta-item">⏱ {prep} {t("min_label")} {t("prep_label")}</span>
      <span class="hb-recipe-meta-item">🔥 {cook} {t("min_label")} {t("cook_label")}</span>
      <span class="hb-recipe-meta-item">🍽 {servings} {t("servings_label")}</span>
    </div>
  </div>
""", unsafe_allow_html=True)
    ...
    with col_nut:
        ...
        if nutrition:
            ...
            st.markdown(f"""
<div class="hb-nut-grid">
  <div>
    <div class="hb-nut-num">{cal}</div>
    <div class="hb-nut-label">{t("calories_label")}</div>
  </div>
  <div>
    <div class="hb-nut-num">{prot}g</div>
    <div class="hb-nut-label">{t("protein_label")}</div>
  </div>
  <div>
    <div class="hb-nut-num">{carbs}g</div>
    <div class="hb-nut-label">{t("carbs_label")}</div>
  </div>
  <div>
    <div class="hb-nut-num">{fat}g</div>
    <div class="hb-nut-label">{t("fat_label")}</div>
  </div>
  <div style="grid-column: 1 / -1;">
    <div class="hb-nut-num">{fiber}g</div>
    <div class="hb-nut-label">{t("fiber_label")}</div>
  </div>
</div>
<div class="hb-nut-source">{src_note}</div>
""", unsafe_allow_html=True)
    ...
    st.markdown("\n".join(step_rows), unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
